...
def _set_csv_field_size_limit() -> None:
    """Raise csv field size limit so exported mega-cases can be parsed."""
    limit = sys.maxsize
    while True:
        try:
...
def _select_text_column(header: List[str], preferred_fields: Iterable[str]) -> str:
    """Pick the first available text column from a CSV header."""
    for field in preferred_fields:
        if field in header:
            return str(field)
    raise ValueError(f"No usable text column in CSV header. Expected one of {list(preferred_fields)}")


def _read_case_log_column(
    case_path: Path,
    preferred_fields: Iterable[str],
    max_lines_per_case: int | None,
) -> List[str]:
    """
    Stream one case CSV and extract the selected text column.

    For very large files, keep the first and last part of the sequence so we
    retain startup context and failure-tail evidence without materializing the
    full CSV in memory.
    """
    _set_csv_field_size_limit()

    with case_path.open("r", encoding="utf-8", errors="replace", newline="") as f:
        reader = csv.reader(f)
        try:
            header = next(reader)
        ...
        text_col = _select_text_column([str(x) for x in header], preferred_fields)
        text_idx = header.index(text_col)

        if max_lines_per_case is None:
            raw_logs: List[str] = []
            for row in reader:
                if text_idx >= len(row):
                    continue
                value = str(row[text_idx]).strip()
                if value:
                    raw_logs.append(value)
            return raw_logs

        limit = max(1, int(max_lines_per_case))
        head_limit = max(1, limit // 2)
        tail_limit = max(0, limit - head_limit)
        head_logs: List[str] = []
        tail_logs = deque(maxlen=tail_limit)
        seen = 0
        ...
        for row in reader:
            ...
            seen += 1
            if len(head_logs) < head_limit:
                head_logs.append(value)
            elif tail_limit > 0:
                tail_logs.append(value)

        if seen <= limit:
            return head_logs + list(tail_logs)
        return head_logs + list(tail_logs)
...
def load_os_preprocessed_cases(cfg: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Adapter for a configured OS preprocessed dataset directory."""
    ...
    if not cases_dir.exists() or not labels_path.exists():
        raise FileNotFoundError(f"OS preprocessed files missing under: {base_dir}")
    ...
    max_lines_per_case = cfg.get("max_lines_per_case", 4000)
    dataset_name = str(cfg.get("name", "OS_preprocessed_B507"))
    ...
    for case_path in tqdm(case_files, desc="load_os_preprocessed"):
        ...
        raw_logs = _read_case_log_column(
            case_path,
            preferred_fields=preferred_fields,
            max_lines_per_case=None if max_lines_per_case is None else int(max_lines_per_case),
        )
    ...
    assert records, "No OS preprocessed cases loaded."
    return records
