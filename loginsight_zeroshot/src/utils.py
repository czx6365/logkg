...
def resolve_optional_path(path_str: Any, base_dir: Path) -> Path | None:
    """Resolve an optional path-like config value; return None for empty values."""
    text = "" if path_str is None else str(path_str).strip()
    if not text:
        return None
